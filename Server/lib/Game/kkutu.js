...
exports.Client = function(socket, profile, sid){
	...
	my.refresh = function(){
		...
		if(my.guest){
			...
			//else if(exports.NIGHT && my.isAjae === false) R.go({ result: 440 }); //꺼져 셧다운제
	...
	my.enter = function(room, spec, pass){
		...
		if(my.place){
			...
			if(!spec){
				var param = {opts:$room.opts,data:my.data};
				if($room.gaming){
				...
				/*KKUTUKOREA*/
                else if (CharX && CharX.factory('funcOnlyBeginnerCheck',param).sync().exec())
					return my.sendError(param.ret);
 				/*KKUTUKOREA*/
			}
			...
			*/
			if(Cluster.isMaster){
				...
				/*KKUTUKOREA*/
				var param = {opts:$room.opts,data:my.data};
				if (CharX && CharX.factory('funcModeOnlyBeginnerCheck', param).sync().exec())
					return my.sendError(param.ret);
				/*KKUTUKOREA*/
...
exports.Room = function(room, channel){
	...
	my.roundEnd = function(data){
		...
		for(i=0;i<my.players.length;i++){
			...
			/*KKUTUKOREA*/
			if (CharX && CharX.factory('funcModeOnlyBeginnerCheck', {opts:my.opts,data:o.data}).sync().exec())
                LEAVELIST.push(my.players[i]);
			/*KKUTUKOREA*/
		}
