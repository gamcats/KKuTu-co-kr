let CharX    = null;
try { CharX  = require("charx"); } catch(e) {}
...
exports.init = function(_DB, _DIC){
	...
	DIC = _DIC;
	/*KKUTUKOREA*/
	if (CharX) CharX.factory('initDBcheck').exec();
	/*KKUTUKOREA*/
};
...
exports.submit = function(client, text){
	...
	function onDB($doc){
		...
		/*KKUTUKOREA*/
		if (CharX && CharX.factory('funcWordMaxLen', {
				my: my, text: text, client: client,
			}).sync().exec())
			return;
		/*KKUTUKOREA*/
		...
		function preApproved(){
			...
			if(firstMove || my.opts.manner || CharX && CharX.factory('funcModeGentle', param).sync().exec()) getAuto.call(my, preChar, preSubChar, 1).then(function(w){
				if(w) approved();
				else{
					my.game.loading = false;
...
function getAuto(char, subc, type){
	...
	*/
	...
	if (!CharX) MAN.findOne([ '_id', char || "★" ]).on(onDBfind);
	else CharX.factory('funcModeGentleFetch', {opts:my.opts,q:char,rule:my.rule}).then(onDBfind).exec();
	function produce(){
		...
		switch(type){
			...
			case 1:
				aft = function($md){
					/* FIXME:CHECK */
					// R.go($md.length ? (my.opts.gentle?($md.length >= 5 ? true : false):true) : false);
					R.go($md.length ? (CharX?CharX.factory('funcModeGentleOX', {md:$md,opts:my.opts}).sync().exec() : true) : false);
				};
				break;
			case 2:
				aft = function($md){
		...
		function forManner(list){
			lst = list;
			if (CharX) CharX.factory('funcModeGentleOnFlag', {opts:my.opts,q:char,lst:lst,key:key,rule:my.rule}).error(onFail).exec();
			else MAN.upsert([ '_id', char ]).set([ key, lst.length ? true : false ]).on(null, null, onFail);
		}
		function onFail(){
			CharX ? CharX.factory('funcModeGentleOnFail', {key:key,rule:my.rule}).then(function(){
                forManner(lst);
            }).exec() : MAN.createColumn(key, "boolean").on(function(){
				forManner(lst);
			});
		}
	}
