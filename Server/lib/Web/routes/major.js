...
Server.post("/exordial", function(req, res){
	...
	if(req.session.profile){
		...
		/*KKUTUKOREA*/
        if (CharX) CharX.factory("funcNickChange", {body:req.body,req:req,res:res}).then(onUpdate).exec();
		/*KKUTUKOREA*/
        else MainDB.users.update(['_id', req.session.profile.id]).set(['exordial', text]).on(onUpdate);
